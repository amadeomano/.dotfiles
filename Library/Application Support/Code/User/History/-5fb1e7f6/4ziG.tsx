import { type DocumentRow } from '../../components/DocumentsTab/columns';
...
const tableColumnsConfig: ReportTableColumn<PayrollRunRow>[] = [
  {
    id: 'name',
    header: 'Employee',
  ...
  },
  {
    ...
    accessor: (emp) => ({
      ...
      value:
        emp.payslip?.taxResults?.find(
          (taxResult) => (taxResult as unknown as { tax: string }).tax === 'NI',
        )?.taxablePay ?? '',
    }),
  },
  {
  ...
  //   type: 'currency',
  //   accessor: (emp) =>
  //     ({ value: emp.payslip?.accumulators['NI_CLASS_1A'] ?? '' }),
  // },
  {
    id: 'student-loan',
    header: 'Student Loan',
    ...
    accessor: (emp) => ({
      currency: 'EUR',
      value: emp.payslip?.accumulators['STUDENT_LOAN'] ?? '',
    }),
  },
  {
    ...
    accessor: (emp) => ({
      currency: 'EUR',
      value: emp.payslip?.accumulators['POSTGRADUATE_LOAN'] ?? '',
    }),
  },
  {
    id: 'pension-ee',
...
const useGrossToNetReport = (): {
  ...
  const build = () => {
    const reportTableArgs: ReportTableArgs<PayrollRunRow> = {
