...
export const REPORT_ID = 'gross-to-net';

export const useGrossToNetReportDocumentRow = (): DocumentRow => {
  ...
  return {
    ...
    download: () => {
      ...
      downloadTableAsCSV({ columns, rows }, `PAY_${key}.csv`);
    },
  };
};

const useGrossToNetReport = (): { build: () => ReportTableArgs } => {
    ...
    > = [
      {
        id: 'name',
        header: 'Employee',
        preparationLookup: (emp) => payrollRunEmployeeFullname(emp.employee),
      },
      {
        id: 'payrollId',
        header: 'Payroll Id',
      ...
      },
      {
        id: 'employeeId',
        header: 'Employee ID',
        preparationLookup: (emp) => String(emp.employeeId),
        defaultHidden: true,
      },
      {
        id: 'ni-number',
      ...
      },
      {
        id: 'tax-code',
        header: 'Tax code',
        // TODO: Tax code in api
        preparationLookup: (emp) => emp.jurisdictionAttributes?.taxCode ?? '–',
      },
      {
        id: 'pay-gross',
        header: 'Gross pay',
        type: 'currency',
        preparationLookup: (emp) => emp.payslip?.grossPay ?? '',
      },
      {
      ...
      },
      {
        ...
        preparationLookup: (emp) =>
          // TODO: taxResults type missing in openapi
          emp.payslip?.taxResults?.find((taxResult) => taxResult.tax === 'NI')
            ?.taxablePay ?? '',
      },
      {
