...
export const TotalMembers = ({ orgUnit, type }: Props) => {
  ...
  const title: string = t('', {
    defaultValue: toTranslate.peopleListLink.totalMembersTitle,
  });
      ...
      <PropertyListItemLabel className={detailsStyles.listRowDescription}>
        ...
        <Tooltip
          content={t('', {
            defaultValue:
              type === 'department'
                ? toTranslate.tooltips.directMembersDepartment
                : toTranslate.tooltips.directMembersTeam,
            name: orgUnit.name.toLocaleLowerCase(),
          })}
        >
          <Icon icon={icons.infoCircle} className={styles.infoCircle} />
        </Tooltip>
      </PropertyListItemLabel>
