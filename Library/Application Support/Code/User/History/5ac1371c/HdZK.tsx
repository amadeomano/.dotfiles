...
const useTotalMembersData = ({
  ...
  const descendantsIds = useMemo(() => {
    const descendants =
      orgUnit.descendants?.map((desc) =>
        getOrgUnitId(desc as OrgUnitResult, type),
      ) ?? [];
    const current = getOrgUnitId(orgUnit, type);
    const result = [...descendants, current].filter(
  ...
  const hasAdminAccess =
    permissionData.data?.data?.orgUnits?.metadata?.accessRights?.permissionsList?.includes(
  ...
  return { orgUnitsData, totalMembersCount, descendantsIds, hasAdminAccess };
};
...
export const TotalMembers = ({ orgUnit, type }: Props) => {
  ...
  const totalMembersData = useTotalMembersData({
    enabled: featureFlag.isReady && featureFlag.isOn,
    orgUnit,
    type,
  });

  const isEnabledAndReady =
    featureFlag.isReady && featureFlag.isOn && totalMembersData.hasAdminAccess;
