...
function offsetBigRectToInclude(smallRect: Rect, bigRect: Rect) {
  // Check if smallRect is completely inside bigRect
  const isInside =
  ...
  if (isInside) {
    // If smallRect is inside bigRect, return the original bigRect
    return {
      x: bigRect.x,
      y: bigRect.y,
      width: bigRect.width,
    ...
    // Calculate the new position for bigRect to include smallRect
    let newX = bigRect.x;
    let newY = bigRect.y;
    ...
    if (smallRect.x < bigRect.x) {
      newX = smallRect.x;
    } else if (smallRect.x + smallRect.width > bigRect.x + bigRect.width) {
      newX = smallRect.x + smallRect.width - bigRect.width;
    }

    // Adjust the y position
    if (smallRect.y < bigRect.y) {
      newY = smallRect.y;
    } else if (smallRect.y + smallRect.height > bigRect.y + bigRect.height) {
      newY = smallRect.y + smallRect.height - bigRect.height;
    }

    // Return the adjusted bigRect
    return { x: newX, y: newY, width: bigRect.width, height: bigRect.height };
  }
}
...
export const useViewportActions = (): UseViewportActionsResult => {
  ...
  const { getNode, getViewport, setViewport, getNodes, fitView } =
  ...
  const fitNodes = useCallback(
    async (
      ...
      if (nodes /*&& treeRef.current*/) {
        ...
        if (
          options?.avoidEmptySpace &&
          zoom === currentZoom &&
          y < transform[1] // exclude collapse animations
        ) {
          const allNodesRect = getRectOfNodes(getNodes(), nodeOrigin);
          const currentViewport = getBoundsFromTransform(
            transform,
            width,
            height,
          );
          const adjustedViewportRect = offsetBigRectToInclude(
            nodesRect,
            currentViewport,
          );
