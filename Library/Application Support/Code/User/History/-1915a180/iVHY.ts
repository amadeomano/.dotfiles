...
const getViewportBoundsFromTransform = (
  ...
  const scaleFactor = zoom * (1 + padding);
  ...
  const boundsX = boundsCenterX - paddedWidth / 2;
  const boundsY = boundsCenterY - paddedHeight / 2;
...
function offsetViewportRectToInclude(viewport: Rect, rect: Rect) {
  const viewportRightEdge = viewport.x + viewport.width;
  const viewportBottomEdge = viewport.y + viewport.height;

  const rectRightEdge = rect.x + rect.width;
  const rectBottomEdge = rect.y + rect.height;

  const isWithinViewportRect =
    rect.x >= viewport.x &&
    rect.y >= viewport.y &&
    rectRightEdge <= viewportRightEdge &&
    rectBottomEdge <= viewportBottomEdge;
  ...
  if (isWithinViewportRect) {
    ...
    if (rect.x < viewport.x) {
      newX = rect.x;
    } else if (rectRightEdge > viewportRightEdge) {
      newX = rectRightEdge - viewport.width;
    }

    if (rect.y < viewport.y) {
      newY = rect.y;
    } else if (rectBottomEdge > viewportBottomEdge) {
      newY = rectBottomEdge - viewport.height;
    }
