...
type Subscribe = ReturnType<typeof useTreeLayoutApi>['subscribe'];
const waitForNodesInitialised = (subscribe: Subscribe) => {
  return new Promise((resolve) => {
    const unsubscribe = subscribe((s, prevS) => {
      ...
      const isInitialised = s
        .getNodes()
        .filter((n) => !n.hidden)
        .every((n) => n[internalsSymbol]?.handleBounds !== undefined);

      const isPrevInitialised = prevS
        .getNodes()
        .filter((n) => !n.hidden)
        .every((n) => n[internalsSymbol]?.handleBounds !== undefined);
      console.log(
...
export const useViewportActions = (): UseViewportActionsResult => {
  ...
  const fitNodes = useCallback(
    async (
      ...
      waitForNodesInitialised(subscribe);
    ...
    [getNode, getViewport, getState, setViewport, subscribe],
  );
