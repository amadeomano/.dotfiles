...
export const OrgChartTree = ({
  ...
  /**
   * hack hack hack
   * currently due to the complete change of displayableHierarchy (e.g. when filtering / spotlighting)
   * the init function gets called several times.
   * for the part that's necessary to run only once we keep track of the absolute first time render
   */
  const isInitialRender = useRef<boolean>(true);

  const restoreViewportFocus = useCallback(async () => {
    if (!focusedEmployeeId && isInitialRender.current) {
      /**
       * hack hack hack:
