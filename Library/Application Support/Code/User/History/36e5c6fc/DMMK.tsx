...
import { OtherPeopleTrigger } from '../ViewActionsBar/components/OtherPeopleTrigger/OtherPeopleTrigger';
import { OpenPositionsWithoutSupervisorButton } from '../ViewActionsBar/components/OpenPositionsWithoutSupervisor';
...
export const OrgChartTree = ({
  ...
  const restoreViewportFocus = useCallback(async () => {
    if (!focusedEmployeeId && !dataSource.isFiltering) {
      /**
       * hack hack hack:
       * set an initial focusedEmployeeId to avoid panning jumps when expanding for the first time
       */
      setFocusedEmployeeId(
  ...
  const handleOnClick = useCallback(
    async (node: HierarchyNode) => {
      ...
      if (nodeIds.length) {
        viewportActions.fitNodes(nodeIds, {
          allowZoomIn: false,
          optionalCentering: true,
        });
      }
    },
    [dataSource.activeNodesState.activeNodeIds, viewportActions.fitNodes],
  );
  ...
  return (
    <div className={styles.orgChartTree} ref={treeRef}>
      ...
      <div className={styles.viewActionsBarWrapper}>
        ...
        <ViewActionsBar metadata={{ testId: TestIds.FloatingActionBar }} />
      </div>
    </div>
  );
};
