...
export const useGbNavigation = () => {
  ...
  const updateURLWithPayrollData = ({
    legalEntityId: overrideLegalEntityId,
    groupId: overrideGroupId,
    period: overridePeriod,
  }: OverrideData) => {
    if (!overrideLegalEntityId) setLegalEntityId(null);
    else
      setLegalEntityId((legalEntityId) =>
        overrideLegalEntityId === legalEntityId
          ? legalEntityId
          : overrideLegalEntityId,
      );

    if (!overrideGroupId) setGroupId(null);
    else
      setGroupId((groupId) =>
        overrideGroupId === groupId ? groupId : overrideGroupId,
      );

    if (!overridePeriod) setPeriod(null);
    else
      setPeriod((period) =>
        overridePeriod === period ? period : overridePeriod,
      );
  };
