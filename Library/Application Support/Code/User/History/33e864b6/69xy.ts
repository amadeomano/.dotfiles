...
export const usePayrollGbBreadcrumbs = () => {
  ...
  const { legalEntityGroups, group } = useCurrentPayrollGroup(legalEntityId);
...
export const useSyncSetup = (initialTab: string) => {
  ...
  const { legalEntities } = useLegalEntities();
  const { legalEntityGroups } = useCurrentPayrollGroup(legalEntityId);
  ...
  useEffect(() => {
    ...
    if (legalEntityGroups.length === 0) {
      ...
      nextGroupId = legalEntityGroups[0].id;
    }
