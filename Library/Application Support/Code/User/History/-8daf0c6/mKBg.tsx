import { useState, ContextType } from 'react';
...
describe('OrgChartTree', () => {
  ...
  const MockOrgChartTree = (props?: Partial<OrgChartTreeProps>) => {
    ...
    const expansionState = useExpansionState(
      props?.isFiltering
        ? props.nodes?.map((node) => node.id) ?? ['0', '1']
        : ['0'],
    );
    ...
    const completeHierarchy: UseHierarchicalDataReturnType<EntityNode> = {
      rootNodes: props?.rootNodes ?? mockRootNodes,
      nodes: props?.nodes ?? mockNodes,
      getNode: props?.getNode ?? ((id: string) => nodesMap.get(id)),
    };

    const dataSource: Partial<DataSource> = {
      isFetching: false,
      hasFetchErrors: false,
      completeHierarchyData: { hierarchy: completeHierarchy } as any,
      displayableHierarchy: completeHierarchy,
        ...
        <PersonCardDataLoaderProvider attributeIds={[]}>
          <OrgChartPreferencesProvider {...mockOrgChartPreferencesProps}>
            <OrgChartDataSourceContext.Provider
              value={dataSource as DataSource}
            >
              <OrgChartTree
