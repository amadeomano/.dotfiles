...
export const previousEmployeeNavigator =
  ...
  (): StepperAction => {
    ...
    if (activeIdx <= 0) return { enabled: false };
    return {
      enabled: true,
      perform: () =>
        navigateToEmployeeDetails(router)(employeeIds[activeIdx - 1]),
...
export const useEmployeeDetailsPanelStepperNavigator = () => {
  ...
  return {
    nextNavigator: navigateToNextEmployee(router, context),
    prevNavigator: navigateToPreviousEmployee(router, context),
  };
};
