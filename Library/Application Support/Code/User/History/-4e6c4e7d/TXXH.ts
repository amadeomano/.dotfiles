...
describe('exportNodes', () => {
  ...
  it('waits for viewport doms to settle before exporting', async () => {
    const onFinish = jest.fn();
    const mockChangingDOMNode = document.createElement('div');
    exportNodes({
      format: ExportFormat.PNG,
      nodes: [mockNodes[0]],
      onFinish,
      DOMNode: mockChangingDOMNode,
      t,
    });
    ...
    setTimeout(() => {
      ...
      mockChangingDOMNode.appendChild(newElement);
    }, 100);

    await waitFor(() =>
      expect(toPng).toHaveBeenCalledWith(
        expect.objectContaining({
          innerHTML: expect.stringContaining('<p>New Element</p>'),
        }),
        expect.anything(),
      ),
    );
