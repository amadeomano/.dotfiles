...
describe('OrgUnitCustomize', () => {
  ...
  describe('CustomizeMenu component', () => {
    ...
    it('does not render leads customisation when feature flag is disabled', async () => {
      ...
      // Verify only 3 switches (description, abbreviation, totalMembers)
      const switches = screen.getAllByRole('switch');
      expect(switches).toHaveLength(3);
    });

    it('allows toggling leads when feature flag is enabled', async () => {
      mockUseFeatureFlag.mockReturnValue({
        isOn: true,
        isReady: true,
      });
      ...
      const switches = screen.getAllByRole('switch');
      // Find the leads switch (should be the 3rd one: description, abbreviation, leads, totalMembers)
      const leadsSwitch = switches[2];

      userEvent.click(leadsSwitch);

      expect(mockCardCustomisations.set).toHaveBeenCalledWith('leads', true);
    });
