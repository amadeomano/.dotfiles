...
import { type EmployeeHeaderData } from '@personio-web/employees-organizations-hook-get-employee-header-data';
import {
  DialogsKeys,
  useDialog,
...
import { useCanRehire } from '@personio-web/employees-organizations-hook-can-rehire';
...
export const useActions = (
  data?: GetEmployeeHeaderData,
  goferData?: EmployeeHeaderData,
) => {
  ...
  // this hook is needed to share logic for showing rehire button across features
  const rehireMetadata = useCanRehire(data);
  ...
  return useMemo(() => {
    ...
    // Use gofer data for end date if feature flag is enabled, otherwise fall back to termination_date
    const terminationDate = goferData?.endDate
      ? goferData.endDate
      : oldTerminationDate;
    ...
    if (employeeId) {
      ...
      if (can_view_orgchart) {
        buttonActions.push({
          ...
          onClick: () => {
            console.log('[] link', employeeId.toString());
            router.push(
      ...
      if (can_impersonate) {
        buttonActions.push({
          ...
          onClick: () => {
            window.location.assign(`/account/login-as-employee/${employeeId}`);
          },
        });
      }

      if (rehireMetadata.canRehire) {
        topDropdownActions.push({
          name: t('rehire-employee'),
          icon: icons.arrowsCirclepath,
          disabled: rehireMetadata.isDisabled,
  ...
  }, [data, isRehiredProfile, rehireMetadata, goferData]);
};
