...
export function useGetPersonCardData(
  ...
  const [employment, setEmployment] = useState<
    EmploymentDataFragment | undefined
  >();
  ...
  const isAccessible = useMemo(
    /*
     * If the principal does not have access to the employment's data, it won't have a preferredName attribute.
     * No error or specific message is send to avoid leaking data.
     */
    () => loading || employment?.person?.preferredName?.value != null,
    [loading, employment],
  );
