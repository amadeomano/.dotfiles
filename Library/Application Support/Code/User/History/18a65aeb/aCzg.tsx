...
describe('OrgUnitCard', () => {
  ...
  describe('Clicking the card with drawer open', () => {
    it('should update drawer when showing different card', () => {
      const mockNode = createMockNode();
      const mockSetExpanded = jest.fn();
      const setActiveCardId = jest.fn();
      ...
      mockUseOrgUnitDetailsState.mockReturnValue({
        state: { orgUnitId: 999, orgUnitType: 'department' },
    ...
    it('should expand when drawer shows same card and card is collapsed', () => {
      const mockNode = createMockNode();
      const mockSetExpanded = jest.fn();
      const setActiveCardId = jest.fn();
      const setDetailsDrawer = jest.fn();
    ...
    it('should collapse when drawer shows same card and card is active', () => {
      const mockNode = createMockNode();
      const mockSetExpanded = jest.fn();
      const setActiveCardId = jest.fn();
      const setDetailsDrawer = jest.fn();
  ...
  describe('Unmatched Org Unit', () => {
      ...
      expect(
        screen.getByText(
          tOrgUnitType('unmatched-description', {
            orgunitType: tOrgUnitType('type_department'),
          }),
        ),
      ).toBeInTheDocument();
    });
    ...
    it('should apply unmatched class to name and description', () => {
      ...
      const description = screen.getByText(
        tOrgUnitType('unmatched-description', {
          orgunitType: tOrgUnitType('type_team'),
        }),
      );
    ...
    it('should not render Leads component when unmatched', () => {
      renderWithContexts(<OrgUnitCard id={mockId} />, {
        displayableHierarchy: {
          getNode: jest
            .fn()
