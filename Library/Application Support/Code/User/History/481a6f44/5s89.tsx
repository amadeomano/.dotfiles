...
const HoverCardReport = ({
  ...
  const { t } = useTranslation('design-system', { keyPrefix: 'hover-card' });
  const { t: td } = useTranslation('dates-core', { keyPrefix: 'common' });

  const firstSectionAvailable = Boolean(
    report?.birthday || report?.anniversary,
  );
  const secondSectionAvailable = Boolean(
    report?.time_off || report?.sick_days || report?.scheduled_time_off,
  );
  const thirdSectionAvailable = Boolean(
    report?.weekly_hours || report?.current_period_extra_hours,
  );
  ...
  return (
    ...
    >
      ...
      {firstSectionAvailable &&
        (secondSectionAvailable || thirdSectionAvailable) && <Divider />}
      ...
      {report?.weekly_hours && (
        <MetadataItem.Value
          ...
          value={td('hours', { hours: report?.weekly_hours })}
        />
      )}
      {report?.current_period_extra_hours && (
        <MetadataItem.Value
          ...
          label={t('overtime')}
          value={td('hours', { hours: report?.current_period_extra_hours })}
        />
      )}
