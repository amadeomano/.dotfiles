...
export const PayRunsTab = () => {
  ...
  return (
    <Table
      isLoading={isPayRunsFetching || tbl.isConfigLoading}
      table={table}
      columnConfig={getColumnsConfig(payGroups?.data)}
      getRowId={({ id }) => id}
      data={data}
    ...
    >
      <ControlBar>
        <Controls.Group />
        <Controls.Sort />
        <Controls.Filter filterConfig={getFiltersConfig(payGroups?.data)} />
        <Controls.Search />
      </ControlBar>
    </Table>
  );
