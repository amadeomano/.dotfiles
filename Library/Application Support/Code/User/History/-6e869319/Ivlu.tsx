...
export const useGetOrgUnitSearchResults = ({
  ...
  const { t } = useTranslation('employees-organizations', {
    keyPrefix: 'org-chart.control-bar.org-unit-search.groups',
  });
  ...
  const searchResults: SearchResultItem[] = useMemo(() => {
    const hierarchyEntityIds = new Set(
      dataSource.completeHierarchyData.data?.hierarchy.nodes.map(
        (node) => node.id,
      ) ?? [],
    );

    const orgUnitOptions = orgUnitItems
      .filter((orgUnit) => hierarchyEntityIds.has(orgUnit.id.id))
      .map(mapOrgUnitOption);
    if (orgUnitOptions.length) {
      const groupKey = prefs.source === 'Department' ? 'department' : 'team';
      orgUnitOptions[0].group = t(groupKey);
    }
    ...
    if (employeeOptions.length) employeeOptions[0].group = t('people');
    ...
    trackSearch,
    prefs.source,
    dataSource.completeHierarchyData.data?.hierarchy.nodes,
  ]);
