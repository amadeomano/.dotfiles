 ...
 * 
 * - hierarchyData: containing the flat hierarchy information that's been fetched & calculated once at the root level
...
const useOrgChartNodes = (): OrgChartNodesContext => {
  ...
  const { isFetching, displayableHierarchy, completeHierarchyData } =
  ...
  useEffect(() => {
    if (!isFetching)
      setExpanded(displayableHierarchy.rootNodes.map((node) => node.id));
  }, [isFetching]);

  // TODO: move filtering mechanism to a level under useOrgChartNodesContext
  useEffect(() => {
    console.log('[] h & f', hierarchyData.loading, preferences.filters.length);
    if (!hierarchyData.loading && preferences.filters.length)
      setExpanded(hierarchyData.hierarchy.nodes.map((node) => node.id));
  }, [hierarchyData.loading, preferences.filters]);
