...
const useListOrgUnits = (type: 'department' | 'team') => {
  ...
  const whiteListedOrgUnitIds = useMemo(() => {
  ...
  }, [orgUnitsData.flattenOrgUnits]);

  const filteredOrgUnits = useMemo(() => {
    const filtered = orgUnitsData.flattenOrgUnits.filter((orgUnit) =>
      whiteListedOrgUnitIds.has(orgUnit.id.id),
    );
    if (type === 'department') console.log('[] filterd', filtered);
    return filtered;
  }, [whiteListedOrgUnitIds]);

  const hierarchy = useHierarchicalData({
    data: filteredOrgUnits,
    config: {
