...
export const usePermission = (
  ...
  const result: UsePermissionResult = {
    isLoading,
    permittedActions: getPermittedActions(data, legalEntityId),
    getResourceActionRights: (resource) => getPermittedActions(data, legalEntityId)
        .find((e) => e.resourceType === resource)
          ?.permittedActions ?? []
      ;
    ,
  };
