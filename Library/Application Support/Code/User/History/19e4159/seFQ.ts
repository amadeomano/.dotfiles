...
describe('navigateToPayRun', () => {
  it('should navigate to a new run if it is different', () => {
    const navigate = navigateToPayRun(mockRouter as );
    navigate('123');
  ...
  it('should not navigate if the run is the same', () => {
    mockRouter.query = { 'active-run': '123' };
    const navigate = navigateToPayRun(mockRouter as NextRouter);
    navigate('123');
...
describe('navigateOutOfPayRun', () => {
  it('should remove the active run from query', () => {
    mockRouter.query = { 'active-run': '123', other: 'value' };
    const navigateOut = navigateOutOfPayRun(mockRouter as NextRouter);
    navigateOut();
    expect(mockRouter.push).toHaveBeenCalledWith({
      query: { other: 'value' },
    });
