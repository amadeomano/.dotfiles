...
async function demonstrateUsage() {
  ...
  try {
    const basicResult = await parser.parseURL('https://example.com', basicConfig);
  ...
  const transformConfig = {
    ...
    socialLinks: {
      selector: 'a[href*="twitter"], a[href*="facebook"], a[href*="linkedin"], a[href*="instagram"]',
      attribute: 'href',
      multiple: true,
      transform: (links) => {
  ...
  try {
    const transformResult = await parser.parseURL('https://github.com', transformConfig);
  ...
  const structuredConfig = {
    ...
    images: {
      ...
      multiple: true,
      transform: (srcs) => {
        return srcs.filter(src => !src.includes('data:'))
                  .map(src => src.startsWith('http') ? src : `https://example.com${src}`);
      }
    }
  };

  try {
    const structuredResult = await parser.parseURL('https://example.com', structuredConfig);
