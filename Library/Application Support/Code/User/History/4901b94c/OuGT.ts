...
function mapEdgesRecursively<T extends BaseData>(
  ...
  treeNodes.forEach((node) => {
    ...
    const coveredActiveChilds: Record<string, number> = {};
    ...
    firstRowChildren.forEach((currentChild, i) => {
      ...
      if (
        ...
        if (activeChildUnder)
          coveredActiveChilds[activeChildUnder.id] =
            coveredActiveChilds[activeChildUnder.id] ?? 0 + 1;

        console.log('[] covered', coveredActiveChilds);
        edges.push(
          createEdge({
            source: node.id,
            ...
            isActive:
              ...
              (!!activeChildUnder &&
                coveredActiveChilds.includes(activeChildUnder)),
            getEdgeStyle,
          }),
        );
      }
