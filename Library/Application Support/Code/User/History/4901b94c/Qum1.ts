...
function mapEdgesRecursively<T extends BaseData>(
  ...
  treeNodes.forEach((node) => {
    ...
    const edgesOverActive: NonNullable<(typeof node)['children']> = [];
    ...
    firstRowChildren.forEach((currentChild, i) => {
      ...
      if (
        ...
        const hasEdgeOverActiveNode = isEdgeOverActiveChild(node, currentChild);
        if (hasEdgeOverActiveNode) edgesOverActive.push(currentChild);
        ...
        edges.push(
          createEdge({
            ...
            isActive:
              ...
              (hasEdgeOverActiveNode && !edgesOverActive.find(currentChild)),
            getEdgeStyle,
          }),
        );
      }
...
function isEdgeOverActiveChild<T extends BaseData>(
  source: TreeNode<T>,
  target: TreeNode<T>,
) {
