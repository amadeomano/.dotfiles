...
function mapEdgesRecursively<T extends BaseData>(
  ...
  treeNodes.forEach((node) => {
    ...
    // counts number of edges above the active child
    ...
    firstRowChildren.forEach((currentChild, i) => {
      ...
      if (
        ...
        (isDirectlyBelowParent && isFirstChild && isLastChild) || // only child of cluster
        (isDirectlyBelowParent && isCurrentChildActive) || // active & center
        (isDirectlyBelowParent && isNodeAboveAnActiveNode(currentChild)) // above a central active
      ) {
