...
export const useTableConfig = (data?: PayrollRun[]) => {
  ...
  return {
    ...
    columnsConfig: getColumnsConfig(router, payGroups?.data),
    filtersConfig: getFiltersConfig(payGroups?.data),
    processedData: getProcessedData(router, table, data, payGroups?.data),
    getGroups: getGroupsFn(router, data, payGroups?.data),
    getGroupData: getGroupDataFn(router, data, payGroups?.data),
  };
};
...
const getColumnsConfig: GetColsConfig = (router, payGroups = []) => [
  columns.titleColumn(router, payGroups),
