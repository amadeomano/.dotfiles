...
describe('useOrgUnitFirstMemberIds', () => {
  ...
  it('should not call goferClient request if orgUnit is defined but has no department id', async () => {
    const mockUndefinedDepartmentOrgUnit = {
      ...mockOrgUnit,
      departmentId: null,
    };

    const { result } = renderHookWithWrapper(
      () => useOrgUnitFirstMemberIds(mockUndefinedDepartmentOrgUnit),
      { authClaim: mockAuthClaim },
    );
