...
const useOrgChartDataSource = ({
  ...
  // Inititalise expansion once all nodes are fetched
  useEffect(() => {
    if (!completeHierarchyData.loading) {
      console.log('[] complete is loaded');
      if (!isFiltering) {
        console.log('[] & complete has NO filters');
      } else if (!filteredIdsData.loading) {
        console.log('[] & complete HAS filters');
      }
    }
  }, [completeHierarchyData.loading, filteredIdsData.loading, isFiltering]);
