...
describe('SmartHoverCardPerson', () => {
  ...
  it('should render department and team with correct org chart link', async () => {
    ...
    const departmentUrl = generateOrgChartLink({
      filters: [
        {
          id: PersonSystemAttribute.Department,
          value: { condition: 'contains', value: ['10'] },
        },
      ],
    });

    const teamUrl = generateOrgChartLink({
      filters: [
        {
          id: PersonSystemAttribute.Team,
          value: { condition: 'contains', value: ['20'] },
        },
      ],
    });

    await waitFor(() => {
      const departmentLink = screen.getAllByRole('link', { name: 'IT' });
      const teamLink = screen.getAllByRole('link', {
        name: 'Design Architecture',
      });
      expect(departmentLink.at(0)?.getAttribute('href')).toBe(departmentLink);
      expect(teamLink.at(0)?.getAttribute('herf')).toBeInTheDocument(teamLink);
    });

    // expect(departmentLink.getAttribute('href')).toBe('hi');
  });
});
