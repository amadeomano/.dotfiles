...
import { type TreeNode } from '../TreeLayout/types';
import {
  nodeTypes,
  nodeWidth,
  treeLayoutStaticProps,
} from '../OrgChartTree/constants';
...
const renderHookWithProvider = (props?: Partial<DataSource>) => {
  const Provider = ({ children }: React.PropsWithChildren) => {
    ...
    const nodes = hierarchy[0].flatMap((node) => node.descendants);

    // Simulate expansion / update of the chart nodes
    useEffect(() => {
      setTimeout(() => setHierarchy(stratify(expandedData)), 0);
    }, []);
...
describe('useViewportActions', () => {
  ...
  describe('fitNodeBranch', () => {
    it('should expand ancestors and set active nodes', async () => {
      ...
      expect(setActiveNodeIds).toHaveBeenCalledWith(['1']);
    ...
    it('should include direct ancestors when specified in options', async () => {
      ...
      await act(async () =>
        result.current.fitNodeBranch(mockNode as any, {
