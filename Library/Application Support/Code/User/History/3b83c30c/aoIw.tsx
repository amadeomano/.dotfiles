...
describe('MembersList', () => {
  ...
  it('should apply correct CSS classes to member option elements', async () => {
    server.use(ListOrgUnitMembersCustomHandlers.defaultHandler);
    ...
    // Verify memberOptionRow and memberOptionContent classes are applied
    const memberOptionRows = container.querySelectorAll('.memberOptionRow');
    expect(memberOptionRows.length).toBeGreaterThan(0);

    const memberOptionContents = container.querySelectorAll(
      '.memberOptionContent',
    );
    expect(memberOptionContents.length).toBeGreaterThan(0);
  });
});
