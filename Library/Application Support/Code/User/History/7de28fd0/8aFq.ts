...
export const usePayrollRunNavigation = () => {
  ...
  const slugs: string[] = router.query.slug;
  ...
  const navigateToListing = () => {
    router.push({
      query: {
        ...router.query,
        slug: (router.query.slug as string[]).slice(0, EMPLOYEE_ID_INDEX),
      },
    });
  };

  const activeEmployeeId = router.query.slug?.at(EMPLOYEE_ID_INDEX);
