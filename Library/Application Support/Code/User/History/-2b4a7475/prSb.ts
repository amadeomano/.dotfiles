...
export function useEmployeePensions(employeeId?: string) {
  ...
  const {
    ...
    requestPathParams: { employeeId: numericEmployeeId },
    enabled: !Number.isNaN(numericEmployeeId),
  });
  ...
  return { employeePensions, isFetching, isError, error };
}
