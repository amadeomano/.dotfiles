...
describe('SourceSwitcher', () => {
  ...
  it('should render with default source when feature flag is enabled', () => {
    ...
    const trigger = screen.getByRole('button', { name: t('people') });
  ...
  it('should render all source options in dropdown', () => {
    ...
    expect(
      screen.getByRole('menuitem', { name: t('people') }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: t('departments') }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: t('teams') }),
    ).toBeInTheDocument();
  });
  ...
  it('should change source when selecting a different option', async () => {
    ...
    userEvent.click(screen.getByRole('button', { name: t('people') }));
    userEvent.click(screen.getByRole('menuitem', { name: t('departments') }));
  ...
  it('should call setOrgUnitDetailsStateWithCallback when org unit details state exists', async () => {
    ...
    userEvent.click(screen.getByRole('button', { name: t('people') }));
    userEvent.click(screen.getByRole('menuitem', { name: t('teams') }));
