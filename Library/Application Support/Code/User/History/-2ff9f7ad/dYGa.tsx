...
import { ListEmploymentsByPersonIdsHandlers } from '@personio-web/employees-organizations-gofer/mocking';
...
describe('OrgChartFeature', () => {
  ...
  it('spotlights an employee, cleans filter state and disable filter menu', async () => {
    ...
    renderWithWrapper(<OrgChart />, {
      features: {
        [FeatureFlags.RELEASE_S4]: 'on',
        [FeatureFlags.ENABLE_SPOTLIGHT]: 'on',
      },
    });
    ...
    console.log(
      '[] options',
      within(spotlightPeoplePicker).getAllByRole('option'),
    );
