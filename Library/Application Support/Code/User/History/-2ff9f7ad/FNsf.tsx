...
import { viewLabelsMap } from './ControlBar/consts/viewsConfig';
import { OrgChart } from './OrgChart';
import { FilterCondition, NodeType, View } from './types';
...
describe('OrgChartFeature', () => {
  ...
  beforeEach(() => {
    ...
    server.use(listPositionsDataHandlers.emptyHandler);
   ...
   * currently on CI it exceeds the 5000ms timeout
   * The reason could be due to the excessive rendering time once nodes source changes
   */
  it.skip('should clear spotlight and focus on node when searched employee is selected', async () => {
    server.use(getEmployeeHierarchyHandlers.defaultHandler);
    server.use(listEmploymentsByPersonIdsHandlers.defaultHandler);
    server.use(getSearchEmployeesHandlers.defaultHandler);
  ...
  describe('Views menu', () => {
    ...
    it('should list all user-dependent views', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(t(viewLabelsMap[View.MyDepartment])),
        ).toBeInTheDocument();
      });

      expect(
        screen.getByText(t(viewLabelsMap[View.MyTeam])),
      ).toBeInTheDocument();
      expect(
        screen.getByText(t(viewLabelsMap[View.MyOffice])),
      ).toBeInTheDocument();
    });
    ...
    it('should not list views which the user has no assignment', async () => {
      ...
      await renderWithWrapper(<OrgChart />);
      ...
      expect(
        screen.queryByText(t(viewLabelsMap[View.MyTeam])),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole(t(viewLabelsMap[View.MyOffice])),
      ).not.toBeInTheDocument();
    });
    ...
    it('selecting a view should update view and filter state', async () => {
      ...
      userEvent.click(screen.getByText(t(viewLabelsMap[View.InternalOnly])));
      ...
      expect(
        screen.getAllByText(t(viewLabelsMap[View.InternalOnly])),
      ).toHaveLength(1);
    ...
    it('updating the filter conditions should clear the view state', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByPlaceholderText(tDS('filter-menu.select-placeholder')),
        ).toBeInTheDocument();
      });
      userEvent.click(
        screen.getByPlaceholderText(tDS('filter-menu.select-placeholder')),
      ...
      const checkboxesBefore = screen
        .getAllByRole('checkbox')
        .filter(
          (checkbox) => checkbox.getAttribute('data-state') === 'checked',
        );

      expect(checkboxesBefore).toHaveLength(1);
      ...
      await waitFor(() => {
        expect(
          screen.getByRole('button', { name: t('views.label') }),
        ).toBeInTheDocument();
      });

      const checkboxesAfter = screen
        .getAllByRole('checkbox')
        .filter(
          (checkbox) => checkbox.getAttribute('data-state') === 'checked',
        );
