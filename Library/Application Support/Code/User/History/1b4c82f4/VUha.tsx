...
export const ListEmployeePensions = ({
  ...
  const { navigateToAllocate, activeEmployeeId } = usePayrollRunNavigation();
  const { employeePensions } = useEmployeePensions(employeeId);
  const { mutateAsync: deleteAllocation } = useWrapMutation(
  ...
  const handleRemoveClick = (id: string) => (event: MouseEvent) => {
    event.stopPropagation();
    const employeeId = Number(activeEmployeeId);
    if (Number.isNaN(employeeId)) return;
    deleteAllocation({ requestPathParams: { employeeId, id } })
      .then(handleSuccess)
      .catch(handleError);
  };
          ...
          <Accordion.Item key={pension.id} value={pension.id}>
            <Accordion.Trigger>
              ...
              <IconButton
                ...
                onClick={handleRemoveClick(pension.id)}
              />
            </Accordion.Trigger>
            <Accordion.Content>
