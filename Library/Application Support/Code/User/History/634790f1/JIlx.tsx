  ...
  describe('Basic Rendering', () => {
    ...
    it('should render member count text with total members count', () => {
      ...
      mockT.mockImplementation((key, options) => {
        ...
        // Handle aria-label with direct and total interpolation
        if (text.includes('{{direct}}') && text.includes('{{total}}')) {
          return text
            .replace('{{direct}}', String(options.direct ?? ''))
      ...
      expect(
        screen.getByRole('region', { name: 'members: 5 direct and 10 total' }),
      ).toHaveTextContent('5 members (10 total)');
    });
    ...
    it('should render "No members" when directMemberCount is 0 but totalMembersCount > 0', () => {
      ...
      expect(
        screen.getByRole('region', { name: 'members: 0 direct and 10 total' }),
      ).toBeInTheDocument();
      expect(mockT).toHaveBeenCalledWith('', {
