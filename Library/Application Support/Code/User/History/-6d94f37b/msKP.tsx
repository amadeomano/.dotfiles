...
export const OrgUnitDetails: React.FC<Props> = ({ id, type, drawerConfig }) => {
  ...
  if (!orgUnit) {
    ...
    if (isError) {
      return <ErrorState onClose={drawerConfig?.onCloseClick} />;
    }
  ...
  return (
    <Drawer drawerRef={panelRef}>
      <Drawer.NavigationBar title={panelTitle}>
        {drawerConfig?.onDeleteClick && (
          <IconButton
            icon={icons.trash}
            metadata={{ testId: TestIds.DeleteButton }}
            variant="ghost"
            aria-label="Remove"
            onClick={drawerConfig.onDeleteClick}
          />
        )}
      ...
      <Drawer.Content title={orgUnit.name}>
        ...
        <div className={styles.drawerBody}>
          <p
            ...
            data-test-id={TestIds.DetailsDescriptionField}
            className={classnames({
              [styles.noDescription]: !orgUnit.description,
            })}
          >
            {orgUnit.description ||
              t('', { defaultValue: toTranslate.noDescription })}
          </p>
          <Stack space="gap-default">
            ...
            >
              {drawerConfig?.showParentOrgUnit && (
                <ParentName orgUnit={orgUnit} type={type} />
              )}
              <Resource orgUnit={orgUnit} />
              <Abbreviation orgUnit={orgUnit} />
              <Layer orgUnit={orgUnit} />
              {drawerConfig?.showSubOrgUnits && (
                <SubOrgUnits orgUnit={orgUnit} type={type} />
              )}
      ...
      {drawerConfig?.onEditClick && (
        <ActionBar>
          <Actions.Primary
            variant="emphasisAccent"
            onClick={drawerConfig.onEditClick}
