...
import { exportNodes, useTreeLayout, ExportFormat } from '../TreeLayout';
...
export const ControlBar = ({
  ...
  const nodesInitialized = useNodes();
  const requestedFormat = useRef<ExportFormat | null>(null);
  console.log(
    '[] check',
    isExporting,
    nodesInitialized,
    requestedFormat.current,
  );
  if (isExporting && nodesInitialized && requestedFormat.current) {
    // exportNodes<HierarchyTreeNode>({
    //   format: requestedFormat.current,
    //   nodes: getNodes(),
    //   t: tErrors,
    // });
    // requestedFormat.current = null;
  }
