...
const getOperationTypes = ({ operations, context }) => {
  ...
  Object.entries(operations).forEach(([operationId, operation]) => {
    ...
    const alreadyTypedRequestParams = [];
    const optionsProps = [];
    ...
    if (operation.hasRequestBody) {
      ...
      alreadyTypedRequestParams.push('data');
    }
    ...
    if (operation.hasQuery) {
      ...
      alreadyTypedRequestParams.push('params');
    }

    if (operation.hasHeaders) {
      key.push(names.headers);
      optionsProps.push(`headers: ${names.headers};`);
      alreadyTypedRequestParams.push('headers');
    }

    const axiosRequestConfigOmitList = alreadyTypedRequestParams
      .map((optionName) => `'${optionName}'`)
      .join(' | ');
    const axiosRequestConfig = alreadyTypedRequestParams.length
      ? `Omit<AxiosRequestConfig, ${axiosRequestConfigOmitList}>`
      : 'AxiosRequestConfig';
    ...
    if (operation.method === 'get') {
      ...
      operationTypes.push(
        `export type ${names.hook} = <Selection = ${names.data}>(
          options${optionalOptions}: { axiosOptions?: ${axiosRequestConfig} } & ${names.options}<Selection>,
      ...
      if (operation.hasPathParams) {
        hookVariables.push(`requestPathParams: ${names.pathParams};`);
      }

      if (operation.hasRequestBody) {
        hookVariables.push(`requestBody: ${names.body};`);
      }

      if (operation.hasQuery) {
        if (operation.queryRequired) {
          hookVariables.push(`requestQuery: ${names.query};`);
        } else {
          hookVariables.push(`requestQuery?: ${names.query};`);
        }
      }
      ...
      if (operation.hasHeaders) {
        hookVariables.push(`requestHeaders: ${names.headers};`);
      }
      ...
      operationTypes.push(
        ...
        AxiosError<${names.errorResponse}>,
        ${
          hookVariables.length
            ? `{${hookVariables.join('')}} & ${axiosRequestConfig}`
            : `${axiosRequestConfig}`
        },
        >;`,
      );
