...
const getOperationTypes = ({ operations, context }) => {
  ...
  Object.entries(operations).forEach(([operationId, operation]) => {
    ...
    const alreadyTypedAxiosRequestParams = [];
    const optionsProps = [];
    ...
    if (operation.hasRequestBody) {
      ...
      alreadyTypedAxiosRequestParams.push('data');
    }
    ...
    if (operation.hasQuery) {
      ...
      alreadyTypedAxiosRequestParams.push('params');
    }

    if (operation.hasHeaders) {
      key.push(names.headers);
      optionsProps.push(`headers: ${names.headers};`);
      alreadyTypedAxiosRequestParams.push('headers');
    }

    const AxiosRequestConfig = alreadyTypedAxiosRequestParams.length
      ? `Omit<AxiosRequestConfig, ${alreadyTypedAxiosRequestParams.join(
          ' | ',
        )}>`
      : 'AxiosRequestConfig';
