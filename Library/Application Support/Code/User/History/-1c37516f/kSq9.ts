...
export const useResolveViewportRequest = (): Viewport | undefined => {
  ...
  const hasExpansion =
    prefs.expansionState.derivedExpandedIds.current?.length ||
    prefs.expansionState.expanded.length;
  ...
  if (requestedState.mode === 'fitNode') {
    ...
    reactFlow.setViewport(viewport, {
      duration: requestedState.animated && hasExpansion ? zoomDuration : 0,
    });
