...
import { getTabbarLocation } from './tabBarLocation';
...
export const useBreadcrumbSchema = (
  ...
  tabBarLocation: number,
) => {
  const currentScrollRef = scrollRef;
  const [breadcrumbDepth, setBreadcrumbDepth] = useState(0);
  console.log('[]', tabBarLocation);
  useEffect(() => {
    const scrollListener = () => {
      const scrollTop = scrollRef.current?.scrollTop;
      let newDepth = 0;
      if (scrollTop && scrollTop > 50 && scrollTop < tabBarLocation) {
        newDepth = 1;
      }
      if (scrollTop && scrollTop >= tabBarLocation) {
        newDepth = 2;
      }
  ...
  }, [breadcrumbDepth, tabBarLocation]);
