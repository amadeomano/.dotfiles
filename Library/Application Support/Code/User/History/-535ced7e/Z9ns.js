...
class HTMLParser {
  ...
  /**
   * Extract data with support for HTML content extraction for nested parsing
   * @param {object} $ - Cheerio object
   * @param {object} extractConfig - Configuration for data extraction
   * @returns {object} Extracted data
   */
  extractDataWithHTML($, extractConfig) {
    const result = {};
    ...
    for (const [key, config] of Object.entries(extractConfig)) {
      try {
        if (typeof config === 'string') {
          ...
          // Advanced configuration
          const { selector, attribute, multiple, transform, extractHTML } = config;
          let elements = $(selector);
          ...
          if (multiple) {
            data = elements.map((i, el) => {
              const $el = $(el);
              if (extractHTML) {
                return $.html($el); // Return HTML content instead of text
              } else if (attribute) {
            ...
            const $el = elements.first();
            if (extractHTML) {
              data = $.html($el); // Return HTML content instead of text
            } else if (attribute) {
              data = $el.attr(attribute);
            } else {
              data = $el.text().trim();
            }
          }
          ...
          // Apply transformation if provided
          if (transform && typeof transform === 'function') {
            data = transform(data, $); // Pass $ as second argument for nested parsing
          }
  ...
  async parseURL(url, extractConfig, options = {}) {
    try {
      ...
      // Check if any config uses extractHTML
      const needsHTMLExtraction = Object.values(extractConfig).some(config => 
        typeof config === 'object' && config.extractHTML
      );
      
      const extractedData = needsHTMLExtraction ? 
        this.extractDataWithHTML($, extractConfig) : 
        this.extractData($, extractConfig);
