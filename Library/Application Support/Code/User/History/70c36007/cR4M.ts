...
type OrgUnitData = { id: string; type: 'department' | 'team' };
type Query = OrgUnitData & { companyId: number };
...
const getOrgUnitData = (orgUnit: OrgUnit): OrgUnitData | undefined => {
  ...
  if (
    ...
    return { id: orgUnit.departmentId.id, type: 'department' };
  } else if (orgUnit.teamId?.__typename === 'protocore_hrteamid_TeamId_v1') {
    return { id: orgUnit.teamId.id, type: 'team' };
  }
...
const batcher = create({
  ...
  resolver: (result, query) =>
    result.data?.membersData?.employmentsList.filter((item) =>
...
export const useOrgUnitFirstMemberIds = (orgUnit: OrgUnit) => {
  ...
  return useQuery({
    queryKey: ['OrgUnitCardFirstMembers', orgUnit.id],
    queryFn: () =>
      orgUnitData &&
