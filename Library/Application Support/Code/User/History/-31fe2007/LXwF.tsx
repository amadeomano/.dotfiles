...
export type { BaseData } from 'designSystem/component/table';
...
export type CurrencyValue = {
  currency: CurrencyCellProps['currency'];
  value: string;
};
export type ReportValue = string | number | boolean | CurrencyValue;

export type ReportTableColumn<T extends BaseData> = Pick<
...
export type ReportTableData<T extends BaseData> = {
  columns: ReportTableColumn<T>[];
  rows: T[];
};

export type ReportTableArgs<T extends BaseData> = ReportTableData<T> & {
  isLoading?: boolean;
  title: string;
  key: string;
};

export const ReportTable = <T extends BaseData>({
  rows,
  columns: rawColumns,
  isLoading,
...
const mapColumnToTableColumnConfig = <T extends BaseData>(
        ...
        case 'currency':
          return (
            <TableCell.Currency
              ...
              value={parseFloat((value as CurrencyValue).value)}
              locale={getLocale()}
              currency={(value as CurrencyValue).currency ?? 'EUR'}
            />
          );
        case 'date': {
          return (
