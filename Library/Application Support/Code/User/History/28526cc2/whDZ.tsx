...
import { EmployeeListNavigatorContext } from '../../PayRollRunDetailsModal';
...
const useSyncEmployeeListNavigatorList = (
  employees: PayrollRun['employeeResults'],
) => {
  const context = useContext(EmployeeListNavigatorContext);
  if (!context) return 'hi';
  // throw new Error(
  //   'useSyncEmployeeListNavigatorList must be used within a EmployeeListNavigatorContextProvider',
  // );

  const employeesRef = useRef(employees);
  if (employeesRef.current !== employees) {
    employeesRef.current = employees;
    context.setEmployeeIds(employees.map((e) => e.employeeId));
  }
  return context;
};
...
export const EmployeesTable = ({
  ...
  useSyncEmployeeListNavigatorList(processedTableData);
