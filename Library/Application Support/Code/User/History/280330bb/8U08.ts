...
export function useGetAdditionalSupervisorAttributes() {
  ...
  const additionalSupervisorAttributesIds = useMemo(() => {
    if (!additionalSupervisorAttributes) return undefined;
    return Object.keys(additionalSupervisorAttributes);
  }, [additionalSupervisorAttributes]);

  return {
    additionalSupervisorAttributes,
    additionalSupervisorAttributesIds,
