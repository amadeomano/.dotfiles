...
describe('createParamHandlers', () => {
  ...
  it('should extract the given keys from the query', () => {
    const { getParams } = createParamHandlers(['key1', 'key2']);
    const query = { key1: 'value1', key2: undefined };
  ...
  it('should call set on searchParams of the url', () => {
    const { setParam } = createParamHandlers(['key1']);
    const url = new URL('http://origin/path');
    url.searchParams.set = jest.fn();
    setParam(url, 'key1', 'newValue');
    expect(url.searchParams.set).toBeCalledWith('key1', 'newValue');
  });

  it('should call set on deleteParam of the url', () => {
    const { deleteParam } = createParamHandlers(['key1']);
    const url = new URL('http://origin/path');
    url.searchParams.delete = jest.fn();
    deleteParam(url, 'key1');
    expect(url.searchParams.delete).toBeCalledWith('key1');
  });
});
