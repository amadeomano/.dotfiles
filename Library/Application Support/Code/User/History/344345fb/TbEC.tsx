...
export const Members = ({ orgUnit }: MembersProps) => {
  ...
  const directMembersCount: number = useMemo(() => {
    return (directMemberData.data ?? []).length;
  }, [directMemberData.data]);

  const hasMembers = useMemo(
    () => (orgUnit.directMemberCount ?? 0) > 0 || (totalMembersCount ?? 0) > 0,
    [directMembersCount, totalMembersCount],
  );
  ...
  const totalMembersIndicator = useMemo(() => {
    if (
      !totalMembersCount ||
      totalMembersCount <= 1 ||
      totalMembersCount === orgUnit.directMemberCount
    )
      return null;
    return t('', {
  ...
  return (
    ...
    >
      {directMembersItems.length && (
        <Facepile
          totalItems={orgUnit.directMemberCount ?? 0}
