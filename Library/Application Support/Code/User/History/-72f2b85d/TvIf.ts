...
type OrgUnitData = { id: number; type: 'department' | 'team' } | undefined;
const getOrgUnitData = (orgUnit: OrgUnit): OrgUnitData => {
  if (orgUnit.type === 'ORG_UNIT_TYPE_UNSPECIFIED') return undefined;

  let id: number | undefined;
  let type: NonNullable<OrgUnitData>['type'] | undefined;
...
export const useOrgUnitTwoMemberIds = (orgUnit: OrgUnit) => {
  ...
  const { id, type } = getOrgUnitData(orgUnit);
  return useQuery({
