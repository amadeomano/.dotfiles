...
export const OrgUnitCard = ({ id }: OrgUnitCardProps) => {
  ...
  const unmatchedType = useMemo(() => {
    if (cardData.data?.type === 'ORG_UNIT_TYPE_DEPARTMENT') return 'department';
    return 'team';
  }, [cardData.status]);
  ...
  const handleOpenDetailsDrawer = useCallback(
    (e: React.MouseEvent) => {
      if (!drawerDetails) e.stopPropagation();
      const data = cardData.data;

      if (!data) return;
      if (data.type === 'ORG_UNIT_TYPE_UNSPECIFIED') return;
    ...
    [drawerDetails, cardData.data, setDetailsDrawer, getOrgUnitIdFromCardData],
  );
  ...
  return useMemo(
    () => (
      ...
      >
        ...
        {!isUnmatched && isDescriptionVisible && (
          <p
            aria-label={t('', {
              defaultValue:
                toTranslate.orgUnitCard.accessibleLabels.description,
              description: hasDescriptionData
                ? cardData.data?.description
                : t('', {
                    defaultValue:
                      toTranslate.orgUnitCard.accessibleLabels.noDescription,
                  }),
            })}
        ...
        {!isUnmatched && isAbbreviationVisible && (
          <p
            aria-label={t('', {
              defaultValue: toTranslate.orgUnitCard.accessibleLabels.code,
              code: hasAbbreviationData
                ? cardData.data?.abbreviation
                : t('', {
                    defaultValue:
                      toTranslate.orgUnitCard.accessibleLabels.noCode,
                  }),
            })}
        ...
        {!isUnmatched && isLeadsVisible && cardData.data && (
          <Leads orgUnit={cardData.data} />
        )}
        {!isUnmatched && cardData.data && <Members orgUnit={cardData.data} />}
        {isUnmatched && (
          <p
            aria-label={t('', {
              defaultValue:
                toTranslate.orgUnitCard.accessibleLabels.description,
              description: t('', {
                defaultValue: toTranslate.orgUnitCard.unmatchedDescription,
                orgunitType: t('', {
                  defaultValue:
                    toTranslate.orgUnitCard.unmatchedType[unmatchedType],
                }),
              }),
            })}
            className={classnames(styles.description, styles.unmatched)}
          >
            {t('', {
              defaultValue: toTranslate.orgUnitCard.unmatchedDescription,
              orgunitType: t('', {
