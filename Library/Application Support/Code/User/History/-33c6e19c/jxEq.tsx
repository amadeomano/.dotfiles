...
import { OrgUnitsEdit } from '@personio-web/employees-organizations-feature-org-units';
...
const orgUnitsEditSpy = jest.spyOn(orgUnitsFeature, 'OrgUnitsEdit');
...
describe('OrgUnitsView', () => {
  ...
  it('should navigate back to list when edit is cancelled', async () => {
    mockRouter.push('/organization/org-units/departments/edit/1');
    renderWithWrapper(<OrgUnits />);
  ...
  it('should navigate to delete when Delete button is clicked in edit mode', async () => {
    mockRouter.push('/organization/org-units/departments/edit/1');
    renderWithWrapper(<OrgUnits />);
  ...
  it('should navigate to details when org unit is successfully edited', async () => {
    mockRouter.push('/organization/org-units/departments/edit/1');
    renderWithWrapper(<OrgUnits />);
    ...
    // Get the props passed to OrgUnitsEdit
    expect(orgUnitsEditSpy).toHaveBeenCalled();
    const props = orgUnitsEditSpy.mock.calls[0][0];

    // Simulate successful edit by calling the callback
    props.onEditSuccess('1');
    ...
    orgUnitsEditSpy.mockRestore();
  });

  it('should navigate back to edit when confirmation is cancelled', async () => {
    const orgUnitsEditSpy = jest.spyOn(
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      require('@personio-web/employees-organizations-feature-org-units'),
      'OrgUnitsEdit',
    );
    ...
    // Simulate cancelling confirmation dialog
