...
jest
  .spyOn(columns.taxYearColumn, 'accessor')
  .mockImplementation((run: PayrollRun) => run.taxYear.toString());
...
describe('Filters', () => {
  ...
  test('taxYearFilter calls predicateByStr', () => {
    ...
    expect(predicateByStr).toHaveBeenCalledWith(mockPayrollRun.taxYear, filter);
  });
});
