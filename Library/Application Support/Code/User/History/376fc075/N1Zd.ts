...
export const useAssignablePayGroups = (): AssignablePayGroup[] => {
  ...
  const assignableGroups: AssignablePayGroup[] = payGroups?.data.map((pg) => ({
    id: pg.id,
    name: pg.name,
    employeeCount: pg.employeeCount,
    faces: pg.employees.map(emp => emp.value).map<FacepileAvatarProps>(empId => getPerson(persons, empId.toString())?.avatar),
  });
};
