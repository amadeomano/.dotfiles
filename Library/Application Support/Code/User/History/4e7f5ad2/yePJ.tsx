...
export const OrgUnitSearch = () => {
  ...
  if (cardData.isFetched && selectedOrgUnitId) {
    ...
    if (data && data.type !== 'ORG_UNIT_TYPE_UNSPECIFIED') {
      ...
      if (orgUnitId && !Number.isNaN(orgUnitId)) {
        openDialog('org-units.details.drawer', {
          orgUnitId,
          orgUnitType,
        });
      }
  ...
  if (toFocusNode) {
    if (toFocusNode.node) {
      ...
      console.log('[] par', toFocusNode.node);

      // Only expand and focus if the node has children
      if (toFocusNode.node.childrenCount > 0)
