...
import {
  useListEmploymentsByPersonIds,
  useInfiniteListEmploymentsByPersonIds,
...
import { mapEmployeeOption, useGetSearchResults } from '../../../hooks';
...
export type ControlBaSpotlightProps = Pick<
  ControlBarProps,
  | 'employeeId'
  | 'spotlight'
  | 'setSpotlight'
...
export const ControlBarSpotlight = ({
  employeeId,
  spotlight,
  setSpotlight,
  ...props
}: ControlBaSpotlightProps) => {
  const shouldSpotlightAutomatically = useRef(!spotlight && employeeId);

  useEffect(() => {
    let timer: NodeJS.Timeout | undefined;
    if (!spotlight && employeeId) {
      setSpotlight(employeeId, 'control-bar-menu');
...
const ControlBarSpotlightMenu = ({
  ...
  spotlightVisibleRelationships,
  setSpotlightVisibleRelationships,
}: Omit<ControlBaSpotlightProps, 'employeeId'>) => {
  ...
  const personIds = useMemo(() => {
    ...
    const nodeIds = hierarchy.nodes
      ...
      .map((node) => node.id);
    if (!spotlight) return nodeIds;

    return nodeIds.filter((id) => id !== spotlight);
  }, [hierarchy?.nodes, spotlight]);

  const {
    data: spotlightedEmployment,
    isFetching: spotlightedEmploymentLoading,
  } = useListEmploymentsByPersonIds({
    queryOptions: { enabled: !!spotlight },
    variables: {
      companyId,
      personIds: [spotlight],
    },
  ...
  const {
    ...
    isFetching: employmentsDataLoading,
    fetchNextPage: employmentsFetchMore,
  } = useInfiniteListEmploymentsByPersonIds({
    queryOptions: {
      enabled: !!personIds?.length,
      getNextPageParam: (lastPage) => lastPage.data?.employments?.nextPageToken,
    },
    variables: {
      companyId,
      personIds,
  ...
  const employments = employmentsData?.pages.flatMap(
    (page) => page.data?.employments?.items,
  );
  const loadedItemsLength = employments?.length ?? 0;

  const employmentsDataMapped = useMemo(() => {
    console.log('[] checking spotlight conditions');
    if (!!spotlight && (spotlightedEmploymentLoading || !spotlightedEmployment))
      return [];
    console.log('[] checking employments conditions');
    if (employmentsDataLoading || !employments) return [];
    console.log('[] checked conditions');

    const spotlightedOption =
      spotlightedEmployment?.data?.employments?.items.map((employment) =>
        mapEmployeeOption({
          id: Number(employment.person?.id || 0),
          name: employment?.person?.preferredName?.value || '',
          avatar: employment?.person?.profilePicUrls?.paths?.large || '',
        }),
      ) ?? [];

    const employmentOptions = employments?.map((employment) =>
      mapEmployeeOption({
        id: Number(employment?.person?.id || 0),
        name: employment?.person?.preferredName?.value || '',
        avatar: employment?.person?.profilePicUrls?.paths?.large || '',
    ...
    const options = spotlightedOption
      ?.concat(employmentOptions)
      .filter(Boolean);
    console.log('[] options', options);
    return options;
  }, [employments, employmentsDataLoading]);
  ...
  const paginationConfig = useMemo(() => {
    return spotlightSearch
      ...
      : {
          ...
          onLoadMore: employmentsFetchMore,
        };
  }, [
    spotlightSearch,
    searchResultsLoading,
  ...
  }, [additionalSupervisorAttributes, spotlight, t]);
  ...
  const selectedRelationships = useMemo(() => {
    ...
    if (spotlightVisibleRelationships.length) {
      return spotlightVisibleRelationships;
    }
