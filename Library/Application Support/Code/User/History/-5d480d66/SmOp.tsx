...
export const ControlBarSpotlight = ({
  ...
  const shouldSpotlightAutomatically = useRef(!spotlight && focusedEmployeeId);
  const { additionalSupervisorAttributeIds, isLoading } =
    useGetAdditionalSupervisorAttributes();

  useEffect(() => {
    let timer: NodeJS.Timeout | undefined;
    if (!spotlight && focusedEmployeeId) {
      setSpotlight(focusedEmployeeId, 'control-bar-menu');
...
const ControlBarSpotlightMenu = ({
  ...
  const personIds = useMemo(() => {
    ...
    return [spotlight, ...nodeIds.filter((id) => id !== spotlight)];
