...
import * as multiSelect from 'designSystem/component/multi-select';
...
jest.mock('designSystem/component/multi-select', () => {
  ...
  // );
  const originalModule = jest.fn(() => ({ MultiSelect: jest.fn() }));
  return {
    ...originalModule,
    MultiSelect: jest.fn(originalModule.MultiSelect),
  };
});
...
describe('OrgUnitsSelect', () => {
  ...
  describe('browser specific', () => {
    ...
    it('should avoid multiselect from auto focus on Safari', async () => {
      const { MultiSelect } = require('designSystem/component/multi-select');
