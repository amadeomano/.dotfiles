...
export function computeTreeCoordinates<T extends BaseData>(
   ...
   * | I     | 2      | 1.5          | 1.5             | 5       |
   */
  traverseTreeBefore(root, validateNode);
...
function validateNode<T extends BaseData>(node: TreeNode<T>): void {
  const requiredProps = [
    'prelim',
    'mod',
    'shift',
  ...
  requiredProps.forEach((prop) => {
    if (!(prop in node)) {
      console.error(`Node ${node.id} is missing required property: ${prop}`);
    }
  });
}
