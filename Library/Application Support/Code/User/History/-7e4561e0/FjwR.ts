...
export function computeTreeCoordinates<T extends BaseData>(
  ...
  // Adjust horizontal spacing dynamically based on node size
  const getNodeWidth = (node: TreeNode<T>) => {
    switch (node.size) {
      case 'medium':
        return nodeSize.width * 1.5;
      case 'large':
  ...
  const horizontalNodeSpacing = (nodeA: TreeNode<T>, nodeB: TreeNode<T>) =>
    getNodeWidth(nodeA) / 2 + getNodeWidth(nodeB) / 2 + spacing.sibling;
  ...
  function computeXCoordinateVariables<T extends BaseData>(
    ...
    if (children?.length) {
      ...
      if (prevSibling) {
         ...
         */
        node.prelim =
          prevSibling.prelim + horizontalNodeSpacing(prevSibling as any, node);
      ...
      if (isClusterNodeInSecondRow(node)) {
         ...
         */
        node.prelim =
          prevSibling.prelim + horizontalNodeSpacing(prevSibling, node);
      }
    }
  ...
  function setCoordinates<T extends BaseData>(node: TreeNode<T>): void {
    ...
    let x = node.x * horizontalNodeSpacing(node, node);
    ...
    if (isSecondRowClusterNode && node.cluster.nodes.length % 2 !== 0) {
      x -= horizontalNodeSpacing(node, node) / 2;
    }
  ...
  function separation<T extends BaseData>(
    ...
    let separation =
      getNodeWidth(leftNode) / 2 +
      getNodeWidth(rightNode) / 2 +
      spacing.sibling;
...
function createRootNode<T extends BaseData>(
  ...
  const initializeNode = (node: TreeNode<T>) => {
    if (node.prelim && node.mod && node.shift && node.change) return;
    node.prelim = 0;
    node.mod = 0;
    node.shift = 0;
    node.change = 0;
