...
export const DirectPeopleListLink = ({ orgUnit, type }: Props) => {
  ...
  const actualMembersCount = allMemberIds.data?.length ?? 0;

  if (actualMembersCount === 0) return null;
  ...
  return (
    <div className={styles.peopleListLinkRow}>
      <h5 role="heading" aria-level={5}>
        {t('members.title', { count: actualMembersCount })}
        <Tooltip
          content={
            type === 'department'
              ? t('tooltips.direct-members_department', { name: orgUnit.name })
              : t('tooltips.direct-members_team', { name: orgUnit.name })
          }
        >
