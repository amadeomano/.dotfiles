...
import {
  OtherPeopleTrigger,
  type OtherPeopleTriggerProps,
} from './OtherPeopleTrigger';
...
const renderTestComponent = (props: Partial<OtherPeopleTriggerProps>) => {
  return act(() => renderWithWrapper(<OtherPeopleTrigger {...props} />));
};

describe('OtherPeopleTrigger', () => {
  it('does not render when hidden people count is 0', async () => {
    await renderTestComponent({ hiddenPeopleCount: 0 });
  ...
  it('renders correctly when hidden people count is greater than 0', async () => {
    ...
    await renderTestComponent({ hiddenPeopleCount: 5 });
