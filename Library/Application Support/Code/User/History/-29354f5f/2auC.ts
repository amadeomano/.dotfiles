...
export const useCompleteHierarchy: UsePersonCompleteHierarchyQuery = (vars) => {
  ...
  const leadsCount = useMemo(
    () =>
      orgUnitsHierarchyData.data?.data?.orgUnits?.orgUnitsList.reduce(
        (acc, orgUnit) => Math.max(acc, orgUnit.directMemberCount),
        0,
      ),
    [orgUnitsHierarchyData.data?.data],
  );
  ...
  return {
    data: {
      ...
      hiddenRootIds: [],
      // TODO [OS-1778]: Add leadsCount once the GraphQL query for [OS-1772] is generated
      // leadsCount: max(orgUnitsHierarchyData.data?.data?.orgUnits?.orgUnitLeadsList.length)
      leadsCount: 3,
    },
