...
import type { BreadcrumbItem } from 'designSystem/component/page-shell';
...
jest.mock('next/router', () => ({
  useRouter: jest.fn().mockReturnValue({
    asPath: '?legalEntityId=123&payGroup=FIXED',
...
describe('usePayGroupsBreadcrumb', () => {
  ...
  it('should return the payGroup from search if it exists (and not navigate)', () => {
    const router = useRouter();
    router.query.payGroup = 'mockedPayGroup';
    router.asPath = '/?payGroup=mockedPayGroup';
  ...
  it('should navigate to the selected payGroup', () => {
    const router = useRouter();
    ...
    const { result } = renderHook(() => usePayGroupsBreadcrumb(mockContext));
    (result.current as BreadcrumbItem).dropdownItems?.[1].onClick();

    expect(router.replace).toBeCalledWithUrl(
      expect.stringContaining('payGroup=2'),
    );
  });
});
