...
type OrgUnitTooltipProps = HiddenCardsTriggerProps & {
  isSupervisorSource: boolean;
};
...
const OrgUnitTooltip = ({
  ...
  isSupervisorSource,
  hiddenCardsCount,
}: React.PropsWithChildren<OrgUnitTooltipProps>) => {
  if (isSupervisorSource) return children;
  return (
    <Tooltip content={`10 cards are hidden for not having direct members`}>
      {children}
    </Tooltip>
  );
};
...
export const HiddenCardsTrigger = ({
  ...
  return (
    ...
    >
      <OrgUnitTooltip isSupervisorSource={isSupervisorSource}>
        <Button
          variant="ghost"
          icon={icons.eyeSlash}
