...
export const PayRollRunDetailsModal = ({
  ...
  const approvePayrunFeedbackScreen =
    !isApprovePayrunLoading && approvePayrunStatus === 'success' ? (
      <ConfirmationScreen
        onClick={() => {
          onCloseDialog();
          navigateOutOfPayRun();
          setShouldRedirectToDocuments(true);
        }}
      />
    ) : !isApprovePayrunLoading && approvePayrunStatus === 'error' ? (
      <ErrorScreen onClick={() => onApprovePayrun(currentPayRun?.id)} />
    ) : null;
