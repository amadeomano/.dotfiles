...
export const useOrgUnitDetailsState = () => {
  ...
  const setQueryValueWithCallback = useCallback(
    (
      ...
      const animationDuration = DEFAULT_ANIMATION_DURATION * 1.5; // 360 ms
      const timeout = setTimeout(onComplete, animationDuration);
  ...
  return [queryValue, setQueryValue, setQueryValueWithCallback] as const;
};
