...
export const PersonCardDataLoaderProvider = ({
  children,
  attributeIds,
}: PropsWithChildren<Pick<PersonCardDataLoaderArgs, 'attributeIds'>>) => {
  const { companyId } = useAuthContext();
  const { additionalSupervisorAttributesData: additionalAttribs } = useOrgChartDataSourceContext();

  additionalAttribs.additionalSupervisorAttributes &&
  additionalSupervisorAttributeIds = Object.keys(
    additionalAttribs.additionalSupervisorAttributes,
  )
  ...
  const personCardDataLoader = useMemo(
    () =>
      getPersonCardDataLoader({
        companyId,
        attributeIds,
        additionalAttribs.additionalSupervisorAttributeIds,
      }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [
      // eslint-disable-next-line react-hooks/exhaustive-deps
      JSON.stringify(additionalAttribs.additionalSupervisorAttributeIds),
      // eslint-disable-next-line react-hooks/exhaustive-deps
