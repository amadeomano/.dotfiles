...
3. Add the feature flag to your local `split.js` file (or enable it on split.io)
```
const flags = {
  'PAYME-101-enable-uk-payroll': 'on',
};
```
3. Open `http://localhost:4200/payroll?hub=gb`
4. If needed, in dev toolbox (wrench in bottom-right), change 'payroll' microfrontend loader to 'local'
5. Also in the dev toolbox in the "Mock Service Worker" tab, change the `listLegalEntitiesHandlers` to `listLegalEntities200Handler__listLegalEntitesUkSuccessResponse` to enable the UK specific legal entities for company 1355607
6. Navigate to the [Manage tab -> Development helpers] to initialize data easily
