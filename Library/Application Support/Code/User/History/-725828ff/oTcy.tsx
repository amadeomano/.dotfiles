...
import { sortByStr, sortByDate } from './sortUtils';
...
export const personColumn = (personData: PersonColumnData[]): ColConfig => ({
  id: COLUMNS.person,
  columnSize: 'mediumSmall',
  icon: icons.personCircle,
  header: 'Person',
  accessor: ({ employeeId }) => employeeId,
  cell: PersonCell(personData),
  sortHandler: (desc) => (a, b) =>
    !personData.length
      ? 0
      : sortByStr(
          getPerson(personData, a.employeeId.toString())?.name,
          getPerson(personData, b.employeeId.toString())?.name,
          desc,
        ),
});
