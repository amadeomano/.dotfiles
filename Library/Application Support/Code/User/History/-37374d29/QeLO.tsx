...
const useHMRCP32Report = (): { build: () => ReportTableArgs<RowData> } => {
  const { period: currentPeriod } = useGbNavigation();
  ...
  const build = () => {
    ...
    const [tax, studentLoan, postGraduateLoan]: string[] = [
      '0',
      sumAccBy('STUDENT_LOAN'),
      sumAccBy('POSTGRADUATE_LOAN'),
    ];

    const [eeNI, erNI, allowance]: string[] = [
    ...
    const rows: RowData[] = [
      {
        rowTitle: 'Tax reference',
        value: getPayeRefNumber(configurationData),
      },
      {
        rowTitle: 'Tax period',
        value: getTaxPeriod(currentPeriod, run.taxYear),
      },
      { rowTitle: 'Tax', value: '?' },
      { rowTitle: 'Student Loan', value: studentLoan },
      { rowTitle: 'Post Graduate Loan', value: postGraduateLoan },
      {
        rowTitle: 'Total',
        value: [tax, studentLoan, postGraduateLoan].reduce(toSum, 0).toString(),
      },
      { rowTitle: 'Employee NI', value: eeNI },
      { rowTitle: 'Employer NI', value: erNI },
      { rowTitle: 'Employment allowance used', value: allowance },
      {
        rowTitle: 'Total',
        value: [eeNI, erNI, allowance].reduce(toSum, 0).toString(),
      },
    ];
