...
describe('useOrgChartDataSourceContext', () => {
  ...
  describe('Additional Relationships', () => {
    ...
    it('should handle additional supervisors and subordinates correctly', async () => {
      ...
      expect(hierarchy.rootNodes).toHaveLength(9);
      ...
      expect(spotlightedNode?.descendants);
      const relatedNodes = result.current.displayableHierarchy.nodes.filter(
        (node) => node.data.group?.relatedNodeId === '1',
      );
      console.log(
        '[] hier',
        inspect(result.current.displayableHierarchy.rootNodes),
      );
    });
  });
});
