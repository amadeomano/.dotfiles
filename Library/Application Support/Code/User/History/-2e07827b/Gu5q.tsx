...
describe('useOrgChartDataSourceContext', () => {
  ...
  describe('expansionState', () => {
    it('should set the expansioState to rootNodes by default', async () => {
      ...
      expect(result.current.expansionState.expanded).toEqual(rootNodeIds);
    });

    it('should set the expansioState to all nodes when filtering applied', async () => {
      server.use(listFilteredEmploymentIdsHandlers.defaultHandler);
      const { result } = renderHookWithProvider({
        filters: [
          {
      ...
      const rootNodeIds = result.current.displayableHierarchy.nodes.map(
        (node) => node.id,
      );
      expect(result.current.expansionState.expanded).toEqual(rootNodeIds);
    });
