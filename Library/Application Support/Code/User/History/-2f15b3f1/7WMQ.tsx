...
import { XeroPeopleTable, handleTableStateChange } from '../XeroPeopleTable';
...
describe('handleFilterStateChange', () => {
  ...
  describe('when sorting', () => {
    it('should set the route URL to match the state', () => {
      const onChange = handleTableStateChange(router, false);
      onChange?.({ sorting: { id: 'id', desc: true } });
    ...
    it('should set the route sort URL to none when its removed from state', () => {
      const onChange = handleTableStateChange(router, true);
  ...
  describe('when filtering', () => {
    ...
    describe.each([
      ...
      it(`should unset ${filterField} filter from route`, () => {
        router.asPath = `?${paramString}`;
        onChange?.({});
        expect(router.replace).toBeCalledWithUrl(
          expect.not.stringContaining(paramString),
        );
      });
    });
  });
