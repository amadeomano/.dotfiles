...
describe('handleFilterStateChange', () => {
  ...
  describe('when sorting', () => {
    it('should set the route URL to match the state', () => {
      const onChange = handleFilterStateChange(router, false);
      onChange && onChange({ sorting: { id: 'id', desc: true } });
    ...
    it('should set the route sort URL to none when its removed from state', () => {
      const onChange = handleFilterStateChange(router, true);
      onChange && onChange({ sorting: undefined });
      expect(router.replace).toBeCalledWithUrl(
        expect.stringContaining('sort=none'),
      );
    });
  });
  ...
  describe('when filtering', () => {
    ...
    describe.each([
      ...
      const paramString = `${
        Array.isArray(filterField) ? filterField[1] : filterField
      }=${
        Array.isArray(filterField) ? filterField[0] : filterField
      };contains;value`;
      it(`should set ${filterField} filter on route`, () => {
        onChange &&
          onChange({
            filters: [
              {
                id: Array.isArray(filterField) ? filterField[0] : filterField,
                value: { value: 'value', condition: 'contains' },
              },
            ],
          });
        expect(router.replace).toBeCalledWithUrl(
          expect.stringContaining(paramString),
        );
      });

      it(`should unset ${filterField} filter from route`, () => {
        router.asPath = `?${filterField}=${filterField};contains;value`;
        onChange && onChange({});
        expect(router.replace).toBeCalledWithUrl(
          expect.not.stringContaining(
            `${filterField}=${filterField};contains;value`,
          ),
        );
      });
    });
