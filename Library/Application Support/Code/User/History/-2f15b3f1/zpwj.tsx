...
describe('handleFilterStateChange', () => {
  ...
  describe('when filtering', () => {
    const onChange = handleFilterStateChange(router, false);
    it('should set the route person filter', () => {
      onChange &&
        onChange({
          filters: [
            { id: 'person', value: { value: 'value', condition: 'contains' } },
          ],
        });
      expect(router.replace).toBeCalledWithUrl(
        expect.stringContaining('person=person;contains;value'),
      );
    });

    it('should unset the route person filter', () => {
      router.asPath = '?person=person;contains;value';
      onChange && onChange({});
      expect(router.replace).toBeCalledWithUrl(
        expect.not.stringContaining('person=person;contains;value'),
      );
    });
  });
});
