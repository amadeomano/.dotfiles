...
const columnSorterMap: (
  router: NextRouter,
  payGroups: PayGroup[],
) => ColumnSorterMap = (payGroups) => ({
  title: {
    sorter: columns.titleColumn(payGroups).sortHandler,
...
export const getProcessedData = (
  ...
  if (sorting)
    processedData.sort(
      columnSorterMap(router, payGroups)[sorting.id as Column].sorter(
        sorting.desc,
      ),
    );
  ...
  return table.filters.filters.reduce(
    (acc, { id, value }) =>
      acc.filter(
        columnSorterMap(router, payGroups)[id as Column].filterer(value),
      ),
    processedData,
  );
};
