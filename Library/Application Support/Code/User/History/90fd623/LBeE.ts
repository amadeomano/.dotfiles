...
type SortFilterHandlerMap = {
  ...
  grouper?: (
    data: PayrollRun[],
    groupArgs: Parameters<FetchGroups>[0],
  ) => Grouping[];
  groupFilterer?: (groupArgs: Grouping) => (run: PayrollRun) => boolean;
};
...
const columnSorterMap: (payGroups: PayGroup[]) => ColumnSorterMap = (
  ...
  schedule: {
    ...
    grouper: (data, { groupId, sort }) => {
      ...
      groups.sort(
        columns.scheduleColumn(payGroups).sortHandler(sort === 'desc'),
      );
      return groups.map((run) => ({
        uniqueId: uuidv4(),
        id: groupId,
        label: columns.scheduleColumn(payGroups).header ?? '',
        value: columns.scheduleColumn(payGroups).accessor(run, 0) as string,
      }));
    },
    groupFilterer: (runs, { value }) =>
      runs.filter(
        (run) => columns.scheduleColumn(payGroups).accessor(run, 0) === value,
      ),
  },
  status: {
    ...
    grouper: (data, { groupId, sort }) => {
      ...
      groups.sort(columns.statusColumn.sortHandler(sort === 'desc'));
      return groups.map((run) => ({
        uniqueId: uuidv4(),
        id: groupId,
        label: columns.statusColumn.header ?? '',
        value: columns.statusColumn.accessor(run, 0) as string,
      }));
    },
    groupFilterer: (runs, { value }) =>
      runs.filter((run) => columns.statusColumn.accessor(run, 0) === value),
  },
  approvedOn: {
  ...
  taxPeriod: {
    ...
    grouper: (data, { groupId, sort }) => {
      ...
      groups.sort(columns.taxPeriodColumn.sortHandler(sort === 'desc'));
      return groups.map((run) => ({
        uniqueId: uuidv4(),
        id: groupId,
        label: columns.taxPeriodColumn.header ?? '',
        value: columns.taxPeriodColumn.accessor(run, 0) as string,
      }));
    },
    groupFilterer: (runs, { value }) =>
      runs.filter((run) => columns.taxPeriodColumn.accessor(run, 0) === value),
  },
  taxYear: {
    ...
    grouper: (data, { groupId, sort }) => {
      ...
      groups.sort(columns.taxYearColumn.sortHandler(sort === 'desc'));
      return groups.map((run) => ({
        uniqueId: uuidv4(),
        id: groupId,
        label: columns.taxYearColumn.header ?? '',
        value: columns.taxYearColumn.accessor(run, 0) as string,
      }));
    },
    groupFilterer: (runs, { value }) =>
      runs.filter((run) => columns.taxYearColumn.accessor(run, 0) === value),
  },
});
