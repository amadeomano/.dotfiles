...
import { type BreadcrumbItem } from 'designSystem/component/page';
...
describe('getSchemasFn', () => {
  ...
  it('should call onSelect when dropdown item is clicked', () => {
    const schemasFn = getSchemasFn(title, legalEntities, tabs);
    const schema = schemasFn(2);

    (schema[1] as BreadcrumbItem).dropdownItems?.[0].onClick();
    expect(legalEntities.onSelect).toHaveBeenCalledWith('entity1');

    (schema[2] as BreadcrumbItem).dropdownItems?.[0].onClick();
    expect(tabs.onSelect).toHaveBeenCalledWith('tab1');
  });
});
