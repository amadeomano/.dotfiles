  ...
  return (
    <Stack space="gap-default" metadata={{ testId: 'org-units-leads-picker' }}>
      <Inline alignVertical="center" space="gap-default">
        <Label title={t('attributes.leads')} />
        {isLeadsPromoTagEnabled && <OrgUnitsLeadsPromoTag />}
      </Inline>
      <p className={styles.subTitle}>
        {t('leads-section.subtitle', { maxAllowedLeads: MAX_ALLOWED_LEADS })}
      </p>
          ...
          <Inline space="gap-default" key={`${index}-${currentValue}`}>
            <div className={styles.pickerContainer}>
              <EmployeePicker
                ...
                size="fullWidth"
                placeholder={t('leads-section.placeholder')}
                value={currentValue}
                onChange={(value) => handlePickerChange(index, value)}
                side="top"
                triggerType="search"
                filters={{ statuses: ['ACTIVE', 'LEAVE', 'ONBOARDING'] }}
              />
            </div>
            ...
            <IconButton
              ...
              aria-label={t('leads-section.remove', { index: index + 1 })}
            />
          </Inline>
        );
      })}
