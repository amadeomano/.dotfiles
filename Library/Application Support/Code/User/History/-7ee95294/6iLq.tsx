...
describe('Column Configurations', () => {
  describe('titleColumn', () => {
    ...
    const payGroups = [{ id: '1', name: 'Fixed salaries' }] as PayGroup[];
    const titleCol = titleColumn(payGroups);
    const expectedTitle = `Fixed salaries · 1 Jan`;
    test('accessor should return a correctly formatted title', () => {
      expect(titleCol.accessor(mockPayrollRun, 0)).toBe(expectedTitle);
    });

    test('cell should render the title correctly', () => {
      const Cell = titleCol.cell;
      render(<Cell value={expectedTitle} row={row} />);
      const link = screen.getByText(expectedTitle);

      expect(link).toBeInTheDocument();
      userEvent.click(link);
      expect(mockRouter.query['active-run']).toBe('12345');
    });

    test('sortHandler should sort the pay runs correctly', () => {
      const sortHandler = titleCol.sortHandler(false);
  ...
  describe('scheduleColumn', () => {
    ...
    test('getGroupData should filter data correctly', () => {
      const run = { payGroupId: '1' } as PayrollRun;
      const truthyArg = { value: 'Monthly' } as Grouping;
      const filter = scheduleCol.getGroupData?.(truthyArg);
      expect(filter?.(run)).toBe(true);

      const falsyArg = { value: 'Weekly' } as Grouping;
      const filterFalse = scheduleCol.getGroupData?.(falsyArg);
  ...
  describe('statusColumn', () => {
    ...
    test('getGroupData should filter data correctly', () => {
      ...
      const filter = statusColumn.getGroupData?.(truthyArg);
      expect(filter?.(run)).toBe(true);

      const falsyArg = { value: 'DRAFT' } as Grouping;
      const filterFalse = statusColumn.getGroupData?.(falsyArg);
      expect(filterFalse?.(run)).toBe(false);
    });
  });
  ...
  describe('taxYearColumn', () => {
    ...
    test('getGroupData should filter data correctly', () => {
      const run = { taxYear: 2024 } as PayrollRun;
      const truthyArg = { value: '2024-2025' } as Grouping;
      const filter = taxYearColumn.getGroupData?.(truthyArg);
      expect(filter?.(run)).toBe(true);

      const falsyArg = { value: '2023-2024' } as Grouping;
      const filterFalse = taxYearColumn.getGroupData?.(falsyArg);
      expect(filterFalse?.(run)).toBe(false);
    });
  });
});
