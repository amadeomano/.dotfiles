...
import {
  useListOrgUnits,
  useInvalidateOrgUnitCardDataQuery,
...
const OrgUnitDetailsDrawer = ({ orgUnitId, orgUnitType, action }: Props) => {
  ...
  const invalidateListOrgUnitsHierarchyQuery =
    useInvalidateListOrgUnitsHierarchyQuery();
  const invalidateOrgUnitCardData = useInvalidateOrgUnitCardDataQuery();
  ...
  const orgUnitUIID = data?.data?.orgUnits?.orgUnitsList[0].id.id;
        ...
        <OrgUnitsEdit
          ...
          drawerConfig={{
            onEditSuccess: () => {
              ...
              invalidateOrgUnitCardData(orgUnitUIID);
              invalidateListOrgUnitsHierarchyQuery();
            },
            onEditCancel: () =>
              setState({ orgUnitId, orgUnitType, action: 'view' }),
          }}
