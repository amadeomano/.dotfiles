...
import { type usePeopleData } from '../../../hooks/usePeopleData';

type PeopleData = ReturnType<typeof usePeopleData>;
export const useTableConfig = (peopleData: PeopleData) => {
  const table = useTable();
  const { payGroups } = usePayGroups();

  return {
    table,
    isConfigLoading: peopleData.isLoading,
    columnsConfig: columnsConfig,
    filtersConfig: getFiltersConfig(payGroups?.data),
    processedData: getProcessedData(table, peopleData.people, payGroups?.data),
  };
};
