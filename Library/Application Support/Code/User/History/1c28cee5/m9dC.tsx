...
export const Members = ({ orgUnit, isUnmatched }: MembersProps) => {
  ...
  const hasAdminAccess =
    dataSource.metadata?.accessRights?.permissionsList?.includes(
      'ROLE_PERMISSION_CAN_EDIT',
    ) ?? false;
  const totalMembersCount = useTotalMembersCount(orgUnit, hasAdminAccess);

  const allMemberIds = useOrgUnitFirstMemberIds(orgUnit);
  const personIds = allMemberIds.data?.map((mem) => mem.personId);
  const twoMemberAvatars = useOrgUnitMemberAvatars(personIds);
  const isLoading = allMemberIds.isLoading || twoMemberAvatars.isLoading;
  ...
  const directMembersItems: FacepileItem[] = useMemo(() => {
    return (twoMemberAvatars.data ?? [])
      ...
      .map((member) => ({
        id: member.id,
        name: member.preferredName?.value ?? undefined,
        src: member.profilePicUrls?.paths?.small ?? undefined,
      }));
  }, [twoMemberAvatars.data]);

  const directMembersCount: number = personIds?.length ?? 0;
    ...
    >
      ...
      >
        ...
        <span>
          ...
          {hasAdminAccess && totalMembersIndicator}
        </span>
      </section>
    </Tooltip>
  );
