...
describe('PayrollLayout', () => {
  ...
  describe('legalEntitiesProp', () => {
    ...
    it('should render the label of the selected entity', () => {
      const LEs = { ...legalEntities, selected: '1' };
      render(<PayrollLayout title="" legalEntities={LEs} />);
      expect(screen.getByText('Legal Entity')).toBeInTheDocument();
    });

    it('should call onSelect when an entity is picked', async async () => {
      render(<PayrollLayout title="" legalEntities={legalEntities} />);
      screen.getByText('Select a legal entity').click();
      screen.getByText('Legal Entity').click();
      expect(legalEntities.onSelect).toHaveBeenCalledWith('1');

      await userEvent.click(screen.getByText('Select a legal entity'));
      await userEvent.click(screen.getByText('Legal Entity'));
    });
  });
