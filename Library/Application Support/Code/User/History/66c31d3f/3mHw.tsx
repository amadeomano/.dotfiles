...
import { initialAttributes, initialCardPreferences } from '../constants';
...
export const useOrgChartPreferences = (): OrgChartPreferences => {
  ...
  console.log('[] val', router.query[OrgChartQueryParamKeys.EMPLOYEE]);
  ...
  const [focusedEmployeeId, setFocusedEmployeeId] = useQueryParamState<string>({
    ...
    hasUnparsedValue: /^\d+$/.test(
      router.query[OrgChartQueryParamKeys.EMPLOYEE]?.toString() ?? '',
    ),
  });
