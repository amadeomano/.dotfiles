...
export const useQueryParamState: UseQueryParamState = <T>(
  ...
  useEffect(() => {
    ...
    const tempParser = getParser('', options.forceParseAsJson);
    console.log(
      '[] has unparsed ',
      options.hasUnparsedValue,
      tempParser,
      params,
    );
    if (parser && params && options.hasUnparsedValue) {
      console.log(
        '[] set state for %s to %s',
        params[key],
        parser.parse(params[key]?.toString() ?? ''),
      );
      setState(parser.parse(params[key]?.toString() ?? '') as T);
    }
