...
export const AllocateContributionGroup = () => {
  const { isInAllocate, activeEmployeeId, navigateToEmployee } =
  ...
  const handleSubmit = ({ contributionType, ...rawRequestBody }: FormData) => {
    ...
    // call useCreateEmployeePension to send form data to the server
    const { mutate: createEmployeePension } = useCreateEmployeePension();

    createEmployeePension(requestBody, {
      onSuccess: () => {
