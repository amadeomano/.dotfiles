...
describe('useGetFilterConditions', () => {
  ...
  describe('filter categorization', () => {
    it('should categorize Department filter as direct when source is Department', () => {
      const mockFilters = [
        {
          id: PersonSystemAttribute.Department,
          value: {
      ...
      renderHookWithPreferences(true, {
        filters: mockFilters,
        source: 'Department',
      });

      // Department filter should be direct, so no indirect filters
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith({
        variables: {
          filterExp: '',
          includeDepartment: true,
    ...
    it('should categorize Team filter as direct when source is Team', () => {
      const mockFilters = [
        {
      ...
      // Team filter should be direct, so no indirect filters
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith({
        variables: {
          filterExp: '',
          includeDepartment: true,
    ...
    it('should categorize Department filter as indirect when source is Team', () => {
      const mockFilters = [
        {
          id: PersonSystemAttribute.Department,
          value: {
      ...
      // Department filter should be indirect when source is Team
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith({
        variables: {
          filterExp: 'department_id in ["dept1"]',
          includeDepartment: true,
          includeTeam: true,
        },
        queryOptions: { enabled: true },
    ...
    it('should categorize Team filter as indirect when source is Department', () => {
      const mockFilters = [
        {
          id: PersonSystemAttribute.Team,
          value: {
      ...
      // Team filter should be indirect when source is Department
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith({
        variables: {
          filterExp: 'team_id in ["team1"]',
          includeDepartment: true,
          includeTeam: true,
        },
        queryOptions: { enabled: true },
    ...
    it('should process legacy IDs from Department source', () => {
      ...
      const mockResponse = {
        data: {
          result: {
            employmentsList: [
              { department: { legacyId: { id: '1' } } },
              { department: { legacyId: { id: '2' } } },
              { department: { legacyId: { id: '1' } } }, // duplicate
            ],
          },
        },
      };
      ...
      const mockResponse = {
        data: {
          result: {
            employmentsList: [
              { department: { legacyId: { id: '1' } } },
              { department: { legacyId: { id: undefined } } },
              { department: { legacyId: { id: null } } },
              { department: null },
            ],
          },
        },
      ...
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith(
        expect.objectContaining({
          variables: expect.objectContaining({
            filterExp: 'workplace_id in ["office1"]',
          }),
        }),
      );
    });
  ...
  describe('complex scenarios', () => {
    it('should handle multiple filters of different types with Department source', () => {
      const mockFilters = [
        {
          id: PersonSystemAttribute.Department,
          value: {
      ...
      // Office and LegalEntity are indirect filters
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith(
        expect.objectContaining({
          variables: expect.objectContaining({
            filterExp:
      ...
      // Department is direct filter, so uses legacy_id
      expect(result.current).toBe(
        'legacy_id in ["dept2"] && legacy_id in ["1"] && type == "Department"',
      );
    });

    it('should handle cross-source filtering with Team source and Department filter', () => {
      const mockFilters = [
        {
          id: PersonSystemAttribute.Team,
          value: {
      ...
      // Department filter is indirect when source is Team
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith(
        expect.objectContaining({
          variables: expect.objectContaining({
            filterExp: 'department_id in ["dept1"]',
      ...
      // Result should use legacy_id for Team (direct) and include indirect filter results
      expect(result.current).toBe(
        'legacy_id in ["team1"] && legacy_id in ["1", "2"] && type == "Team"',
      );
    });

    it('should handle cross-source filtering with Department source and Team filter', () => {
      const mockFilters = [
        {
          id: PersonSystemAttribute.Department,
          value: {
      ...
      // Team filter is indirect when source is Department
      expect(mockUseFilterOrgUnitsByEmployees).toHaveBeenCalledWith(
        expect.objectContaining({
          variables: expect.objectContaining({
            filterExp: 'team_id in ["team1"]',
      ...
      // Result should use legacy_id for Department (direct) and include indirect filter results
      expect(result.current).toBe(
        'legacy_id in ["dept1"] && legacy_id in ["1", "2"] && type == "Department"',
      );
    });
  });
});
