...
const ConfigurationAlert: React.FC<ConfigurationAlertProps> = ({ title }) => {
  const { t } = useTranslation(TRANSLATION_NAMESPACE, {
    keyPrefix: 'athree.hub.configuration-alerts',
  });
  const router = useRouter();

  const goToSettings = () => {
    router.push(
      `/configuration/payroll-integration/a3?legalEntityId=${legalEntityId}`,
    );
  };
...
export const NoCompanyMappedAlert = () => {
  const { t } = useTranslation(TRANSLATION_NAMESPACE, {
    keyPrefix: 'athree.hub.configuration-alerts',
  });

  return <ConfigurationAlert title={t('no-company-mapped-title')} />;
};

export const NoWorkplaceMappedAlert = () => {
  const { t } = useTranslation(TRANSLATION_NAMESPACE, {
    keyPrefix: 'athree.hub.configuration-alerts',
  });

  return <ConfigurationAlert title={t('no-workplace-mapped-title')} />;
};
