...
export const OrgUnitsEdit = ({
      ...
      },
      {
        onSuccess: (response) => {
          if (response.data?.orgUnit) {
            ...
            if (orgUnitId) drawerConfig.onEditSuccess(orgUnitId);
          } else if (response.errors) {
            handleMutationServerErrors(response.errors, setError);
          }
        },
  ...
  const onCancelConfirmation = useCallback(() => {
    ...
    closeDialog();
    drawerConfig.onEditConfirmationCancel?.();
  }, [closeDialog, drawerConfig.onEditConfirmationCancel]);

  const onCancel = useCallback(() => {
    reset();
    drawerConfig.onEditCancel();
  }, [reset, drawerConfig.onEditCancel]);
