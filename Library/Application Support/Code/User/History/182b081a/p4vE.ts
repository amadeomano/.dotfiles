...
const getHref = (router: NextRouter) => (routeName: ManageRoute) =>
  format({ pathname: router.pathname, query: { slug: ['manage', routeName] } });
...
export const useManageTabNavigator = () => {
  ...
  return {
    RoutedPage,
    isRouteSelected: isRouteSelected(router),
    navigateTo: navigateTo(router),
  };
};
