...
import { ListEmploymentsByPersonIds } from '@personio-web/employees-organizations-gofer';
...
export type PersonCardDataLoader = DataLoader<string, EmploymentDataFragment>;
...
function getPersonCardDataLoaderBatchFn({
  ...
  return async function personCardDataLoaderBatchFn(
  ...
  ): Promise<EmploymentDataFragment[]> {
    if (personIds.length === 0) {
      return [];
    }

    // Execute a GraphQL query to fetch employment data for the given person IDs.
    const {} = getQueryKeyForDocument({
      document: ListEmploymentsByPersonIds,
      variables: {},
    });
    ...
    const employmentsMap = result.data.employments?.items?.reduce(
      (rest, employment) => ({
        ...rest,
...
): DataLoader<string, EmploymentDataFragment> =>
