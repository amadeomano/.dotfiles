...
import { ExportLimitationSafariDialog } from './components/ExportLimitationSafariDialog';
...
export const ControlBar = ({
  ...
  const handleExportSelection = (format: ExportFormat) => {
    ...
    if (isSafari && [ExportFormat.PNG, ExportFormat.PNG_BG].includes(format)) {
      openDialog('org-chart.export-limitation-safari', {});
      console.log('[] is Unsupported format:', format, openDialog);
      return;
    }
