...
function parseMeaning(meaningHtml, $parent) {
  console.log(
    "Processing meaning HTML length:",
    meaningHtml ? meaningHtml.length : 0
  );
  ...
  return {
    num:
      ...
      null,
    word:
      $("h2 strong").text().trim() || $("h2 strong").attr("content") || null,
    ...
    rawHtml:
      meaningHtml.substring(0, 200) + (meaningHtml.length > 200 ? "..." : ""),
  };
}
...
async function main() {
  ...
  const config = {
    ...
    meanings: {
      ...
      transform: (meaningHtmlArray, $) => {
        ...
        return meaningHtmlArray.map((meaningHtml) =>
          parseMeaning(meaningHtml, $)
        );
      },
    },
  };
  ...
  try {
    ...
    console.log(
      `Attributes: ${result.data.attrs ? result.data.attrs.length : 0}`
    );
    console.log(
      `Meanings found: ${
        result.data.meanings ? result.data.meanings.length : 0
      }`
    );
  } catch (error) {
